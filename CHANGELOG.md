...
- Three endpoints closing the [api#77](https://github.com/ligate-io/ligate-api/issues/77) marketplace-surface gap on top of the bounty + contract event ingestion that shipped in #79 / #81. `GET /v1/contracts/matching/{address}` returns the v0 contract matching set (every `open` contract the address did not post), wrapped in the same `{address, matches: []}` envelope as `/v1/bounties/matching`. v0 has no off-chain tag store and no `commits` table, so the matching signal collapses to "every open contract you didn't post" (the chain rejects self-commits, so excluding the poster gives the actionable set without false positives); tag/skill-mediated narrowing per `docs/protocol/contract-primitive.md` lands when the tag store + commits table follow up. `GET /v1/addresses/{addr}/bounties` and `GET /v1/addresses/{addr}/contracts` are buyer-dashboard surfaces (bounties / contracts posted by a given address), paginated with the same compound `(posted_at_slot, id)` cursor as the global `/v1/bounties` and `/v1/contracts` lists. The bounties surface honours optional `?status=` and `?board=` filters; the contracts surface honours optional `?status=` and `?arbiter=` filters (the path already pins `poster`, so the global list's `?poster=` is implicit here). Implementation reuses the existing `bounties_page` / `contracts_page` queries, with a new `poster` parameter added to `bounties_page` for symmetry with `contracts_page` (existing `/v1/bounties` callers pass `None` and keep their current behaviour). New types: `ContractMatchRow` in queries, `ContractMatchingResponse` + `ContractMatchEntry` in responses. Cached at `TTL_MODEST_SECS` like the rest of the marketplace surface. Powers themisra-dashboard [#79](https://github.com/ligate-io/themisra-dashboard/issues/79) ("my postings" tabs) + [#84](https://github.com/ligate-io/themisra-dashboard/issues/84) (marketplace discovery), Mneme [#1](https://github.com/ligate-io/ligate-mneme/issues/1) (post-attest contract panel), and the ligate-cli #48 `list-matching` subcommands.
